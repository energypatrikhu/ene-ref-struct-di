		...
		function test(structType, testNumber) {
			describe('Struct test' + testNumber, function () {
				it('should have its `size` matching sizeof()', function () {
					...
					assert.equal(expectedSize, structType.size, 'test' + testNumber + ': sizeof(): expected ' + structType.size + ' to equal ' + expectedSize);
				});
				it('should have its `alignment` matching __alignof__()', function () {
					var expectedAlignment = bindings['test' + testNumber + ' alignof'];
					assert.equal(expectedAlignment, structType.alignment, 'test' + testNumber + ': __alignof__(): expected ' + structType.alignment + ' to equal ' + expectedAlignment);
				});
				Object.keys(structType.fields).forEach(function (name) {
					it('should have a matching offsetof() for "' + name + '"', function () {
						...
						assert.equal(expectedOffset, offset, 'test' + testNumber + ': offsetof(' + name + '): expected ' + offset + ' to equal ' + expectedOffset);
					});
				});
			});
		}
